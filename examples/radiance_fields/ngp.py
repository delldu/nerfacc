...
class NGPDensityField(torch.nn.Module):
    ...
    def __init__(
        ...
        # self = NGPDensityField(
        #   (mlp_base): NetworkWithInputEncoding(n_input_dims=3, n_output_dims=1, seed=1337, dtype=torch.float16, 
        #     hyperparams={'encoding': {'base_resolution': 16, 'hash': 'CoherentPrime', 'interpolation': 'Linear', 
        #     'log2_hashmap_size': 17, 'n_features_per_level': 2, 'n_levels': 5, 'otype': 'Grid', 
        #     'per_level_scale': 1.6817928552627563, 'type': 'Hash'}, 
        #     'network': {'activation': 'ReLU', 'n_hidden_layers': 1, 'n_neurons': 64, 'otype': 'FullyFusedMLP', 
        #         'output_activation': 'None'}, 'otype': 'NetworkWithInputEncoding'})
        # )

    def forward(self, positions: torch.Tensor):
        # tensor [positions] size: [4096, 128, 3], min: -2.838365, max: 2.820854, mean: 0.030345
        if self.unbounded: # False
